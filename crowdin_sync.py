...
# directly to Cardinal-AOSP Github.
#
...
def push_as_commit(base_path, path, name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push('git@github.com:Cardinal-AOSP/%s' % (name),
                      'HEAD:%s' % branch)
...
def upload_crowdin(branch, no_upload=False):
    if no_upload:
        print('Skipping translations upload')
        return

    print('\nUploading Crowdin translations')
...
def main():
    ...
    base_path = os.getenv('CARDINAL_CROWDIN_BASE_PATH')
    if base_path is None:
        cwd = os.getcwd()
        print('You have not set CARDINAL_CROWDIN_BASE_PATH. Defaulting to %s' % cwd)
        base_path = cwd
    else:
        base_path = os.path.join(os.path.realpath(base_path))
    if not os.path.isdir(base_path):
        print('CARDINAL_CROWDIN_BASE_PATH + branch is not a real directory: c'
              % base_path)
        sys.exit(1)
